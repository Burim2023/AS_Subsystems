...
void QRCodeReaderCommand::Initialize() {
    ...
    qrTab->PutString("Mode", 
        m_mode == ReadMode::SINGLE_READ ? "Single Read" :
        m_mode == ReadMode::CONTINUOUS_READ ? "Continuous" : "Timed Read"
    );
...
void QRCodeReaderCommand::Execute() {
    ...
    if (++debugCounter % 100 == 0) { // Assuming 50Hz execution
