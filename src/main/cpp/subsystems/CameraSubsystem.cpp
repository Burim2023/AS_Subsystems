...
// Global variables for Orbbec camera
static ob::Context *obContext = nullptr;
static ob::Pipeline *pipeline = nullptr;
...
void CameraSubsystem::Start() {
  ...
  try {
    ...
    pipeline = new ob::Pipeline(depthCamera);
  ...
  while (m_running.load() && cameraRunning.load()) {
    ...
    if (appleFound) {
      ...
      AppleResult largestApple = *std::max_element(appleResults.begin(), appleResults.end(),
        [](const AppleResult &a, const AppleResult &b) { return a.rect.area() < b.rect.area(); });
      ...
      switch(largestApple.color) {
        case 1: drawColor = cv::Scalar(0, 0, 255); break;   // Red apple = red square
        case 2: drawColor = cv::Scalar(0, 255, 255); break; // Yellow apple = yellow square  
        case 3: drawColor = cv::Scalar(0, 255, 0); break;   // Green apple = green square
        default: drawColor = cv::Scalar(255, 255, 255); break; // White for unknown
      }
      ...
      for (const auto& apple : appleResults) {
        if (apple.rect != largestApple.rect) {
          ...
          switch(apple.color) {
            case 1: debugColor = cv::Scalar(0, 0, 128); break;   // Dark red
            case 2: debugColor = cv::Scalar(0, 128, 128); break; // Dark yellow
            case 3: debugColor = cv::Scalar(0, 128, 0); break;   // Dark green
